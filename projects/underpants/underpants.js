...
_.last=function(array, number) {
    ...
    else { return array.slice(array.length-number)
    }
}
...
*   3) This is the logical inverse if _.filter(), you must use _.filter() in your 
* implementation
* Examples:
*   _.reject([1,2,3,4,5], function(e){return e%2 === 0}) -> [1,3,5]
*/
_.reject=function(array,func2){
...
_.partition=function(array, func) {
    var newArray=[[],[]]
    for (let i=0; i<array.length; i++) {
        if (Boolean(func(array[i], i, array))===true) {
...
*
*/
_.map=function(coll,action) {
    var newArray=[];
    _.each(coll, function(element,index,coll){
            newArray.push(action(element,index,coll))
    })
    return newArray;    
}
...
_.reduce=function(array, func, seed) {
    if (seed || seed===0 ) {
        var result=func(seed, array[0], 0);
        for (let i=1; i<array.length; i++) {
            var result=func(result, array[i], i)
        }
    }
    else {
        var result=func(array[0], array[1], 1);
...
_.extend=function(){
    ...
    var reduce=function(array, seed, func) {
        if (seed || seed===0 ) {
            var result=func(array[0], array[1], 1);
            for (let i=2; i<array.length; i++) {
                var result=func(result, array[i], i)
    ...
    const objCopy=function(one, two, three) {
        for (var key1 in one) {
            for (var key2 in two) {
                if (key1===key2) {
                    one[key1]=two[key2];
